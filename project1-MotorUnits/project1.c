...
// ARMS 
double Kp_arm =  600.0;//100.0;
double Kd_arm =  11.2;//18.9;
double passive_Kd_arm = 1.0;

// BASE TRANSLATION
double Kp_base_trans = /*185.0;*/400.0;
double Kd_base_trans = /*36.0;*/36.9; 
double passive_Kd_base_trans = 2.0;

// BASE ROTATION
double Kp_base_rot =  200.0;
double Kd_base_rot =  12.8;
...
double time;
{ 
  double Fx, error[2], trans_error, trans_vel, theta_error;
  ...
  if (ACTUATE_BASE) {
    ...
    //printf("%lf %lf %lf %lf\n", time, error[X], error[Y], theta_error);
  }
  else {
